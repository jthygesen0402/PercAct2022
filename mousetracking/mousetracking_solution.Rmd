...
# loading in data
d <- read_csv('Mousetracking_experiment/subject-3.csv')

d <- d %>% mutate(correct_figure = ifelse(correct_button=="left_button", left_figure,right_figure))
d <- d %>% mutate(congruent= ifelse(correct_figure=="kiki"& sound=="A7"|correct_figure=="bouba"& sound=="A1", "congruent","incongruent"))
```
